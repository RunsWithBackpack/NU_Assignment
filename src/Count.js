  ...
  render() {
    ...
    return (
      <TasksWrapper>
        ...
        <MyItems>My items ({this.props.tasks.length})</MyItems>
        <HrLine />
        <TableHeaderWrapper>
        ...
        { this.props.tasks.length > 0 && this.props.tasks.map((task, idx) => {
            ...
            return (
              <TaskListItemOdd key={task.name}>
                <IndivTaskColumn flexsize='2'>{task.name}</IndivTaskColumn>
                <IndivTaskColumn>{task.timestamp}</IndivTaskColumn>
                <IndivTaskColumn flexsize='0.5'><DeleteButton onClick={() => this.handleClick(task.name)}>X</DeleteButton></IndivTaskColumn>
              </TaskListItemOdd>
